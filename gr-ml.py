...
ml_output_file = Path("data/gr-ml.txt")
...
def create_sequences(data, seq_length=24):
    ...
    for i in range(len(data) - seq_length):
        ...
        for j in range(seq_length):
            seq.append(create_features(data[i + j]))
        X.append(seq)
        y_wind.append(float(data[i + seq_length]["wind_speed"]))
        y_solar.append(float(data[i + seq_length]["irradiance"]))
    return np.array(X), np.array(y_wind), np.array(y_solar)

def train_gru_model(hourly_data):
    ...
    X, y_wind, y_solar = create_sequences(train_data, seq_length)
    ...
    y_wind_scaled = wind_scaler.fit_transform(y_wind.reshape(-1, 1)).flatten()
    
    solar_scaler = StandardScaler()
    y_solar_scaled = solar_scaler.fit_transform(y_solar.reshape(-1, 1)).flatten()
    
    # Build GRU model for wind
    wind_model = Sequential([
        GRU(64, activation='relu', input_shape=(seq_length, 6), return_sequences=True),
        Dropout(0.2),
    ...
    wind_model.fit(X_scaled, y_wind_scaled, epochs=10, batch_size=32, verbose=0)
    
    # Build GRU model for solar
    ...
    solar_model.fit(X_scaled, y_solar_scaled, epochs=10, batch_size=32, verbose=0)
    
    return wind_model, solar_model, scaler, wind_scaler, solar_scaler

def predict_daily_avgs(wind_model, solar_model, scaler, wind_scaler, solar_scaler, hourly_data):
    # Get last 24 hours from training data to use as initial sequence
    train_data = [r for r in hourly_data if parse_date(r["timestamp"]).date() <= parse_date("2026-02-20 23:59:59").date()]
    last_sequence = train_data[-24:] if len(train_data) >= 24 else train_data
    
    # Predict for Feb 21-28 2026
    dates = ["2026-02-21", "2026-02-22", "2026-02-23", "2026-02-24", "2026-02-25", "2026-02-26", "2026-02-27", "2026-02-28"]
    predict_data = []
    
    current_seq = [create_features(r) for r in last_sequence]
    
    for i, date in enumerate(dates, 1):
        # Generate 24 hourly predictions for this date
        daily_wind = []
        daily_solar = []
        ...
        for hour in range(24):
            ...
            # Prepare sequence
            seq = np.array([current_seq]).astype(np.float32)
            seq_scaled = scaler.transform(seq.reshape(-1, 6)).reshape(1, -1, 6)
            
            # Predict
            wind_pred_scaled = wind_model.predict(seq_scaled, verbose=0)[0, 0]
            solar_pred_scaled = solar_model.predict(seq_scaled, verbose=0)[0, 0]
            
            # Inverse transform
            wind_pred = wind_scaler.inverse_transform([[wind_pred_scaled]])[0, 0]
            solar_pred = solar_scaler.inverse_transform([[solar_pred_scaled]])[0, 0]
            
            daily_wind.append(max(0, wind_pred))
            daily_solar.append(max(0, solar_pred))
            
            # Update sequence
            current_seq = current_seq[1:] + [features]
        
        wind_min, wind_avg, wind_max = min(daily_wind), np.mean(daily_wind), max(daily_wind)
        solar_min, solar_avg, solar_max = min(daily_solar), np.mean(daily_solar), max(daily_solar)
...
def generate_html(ml_daily, nasa_daily, metrics_text, total_api_rows):
    """Generate hardcoded gr-ml.html with data from gr-ml.txt"""
    html_file = Path("gr-ml.html")
...
<head>
  ...
  <title>GRU-ML | Energy Data Summary</title>
...
<body>
  <div class="container">
    <header class="header">
      <h1>GRU-ML: Predict vs Actual</h1>
      <nav class="header-nav">
        ...
        <a href="xg-ml.html" class="header-nav-link">XGBoost-ML</a>
    ...
    <div class="main-summary">
      <div class="summary-cards">
        <div class="summary-card" style="grid-column: 1 / -1;">
          <div class="summary-label">GRU Performance & Config</div><table style="width: 100%; border-collapse: collapse; font-size: 0.88em;">
            <thead>
...
<tr style="border-top: 2px solid rgba(255,255,255,0.1); padding: 8px 0;">
  ...
  <td style="text-align: left; font-weight: 600; color: #06b6d4; padding-left: 10px;" colspan="4">GRU (Gated Recurrent Unit)</td>
 </tr>
...
def main():
    ...
    print(f"\nTraining GRU model using {len(all_data)} total rows from all *api.txt files...")
    wind_model, solar_model, scaler, wind_scaler, solar_scaler = train_gru_model(all_data)
    
    ml_daily = predict_daily_avgs(wind_model, solar_model, scaler, wind_scaler, solar_scaler, all_data)
    ...
    with open(ml_output_file, "w") as f:
        ...
        f.write(f"# Summary: Training data = {total_api_rows} | full-api actual = {len(nasa_daily)} | GRU predict = {len(ml_daily)}\n")
        f.write("[full-api]\n\n")
        
        # Data-A Header
        f.write("# Data-A: GRU-ML Predictions (Feb 21–28 2026)\n")
    ...
    print(f"✅ gr-ml.txt generated with GRU ML (Data-A/B + metrics)")
    print(f"✅ gr-ml.html generated with hardcoded data from gr-ml.txt")
