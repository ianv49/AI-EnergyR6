...
ml_output_file = Path("data/nf-ml.txt")
...
def predict_naive_daily_avgs(hourly_data):
    """
    Naive Forecast (Persistence Model): Predict Feb 21-28 2026 using last observed values
    Strategy: For each day in prediction period, use the last 24 hours from training data
    to predict the daily values (min/avg/max)
    """
    # Sort data by timestamp to ensure chronological order
    sorted_data = sorted(hourly_data, key=lambda r: parse_date(r["timestamp"]))
    
    # Get data up to Feb 20 2026 (last training day)
    train_end = parse_date("2026-02-20 23:59:59")
    training_data = [r for r in sorted_data if parse_date(r["timestamp"]) <= train_end]
    
    # Extract last 24 hours of training data (Feb 20 2026)
    last_24h_wind = []
    last_24h_solar = []
    
    for row in training_data[-24:]:  # Last 24 rows should be Feb 20
        last_24h_wind.append(float(row["wind_speed"]))
        last_24h_solar.append(float(row["irradiance"]))
    
    # If we don't have exactly 24 hours, still proceed
    if len(last_24h_wind) > 0:
        wind_min_last = min(last_24h_wind)
        wind_avg_last = np.mean(last_24h_wind)
        wind_max_last = max(last_24h_wind)
        solar_min_last = min(last_24h_solar)
        solar_avg_last = np.mean(last_24h_solar)
        solar_max_last = max(last_24h_solar)
    else:
        # Fallback if no data
        wind_min_last = wind_avg_last = wind_max_last = 0
        solar_min_last = solar_avg_last = solar_max_last = 0
    
    # Predict Feb 21-28 by persistence (repeating last observed pattern)
    ...
    for i, date in enumerate(dates, 1):
        predict_data.append({
            "id": str(i),
            "timestamp": date,
            "wind-min": f"{wind_min_last:.2f}",
            "wind-avg": f"{wind_avg_last:.2f}",
            "wind-max": f"{wind_max_last:.2f}",
            "solar-min": f"{solar_min_last:.2f}",
            "solar-avg": f"{solar_avg_last:.2f}",
            "solar-max": f"{solar_max_last:.2f}",
            "source": "NF-ML"
        })
...
def generate_html(ml_daily, nasa_daily, metrics_text, total_api_rows):
    """Generate hardcoded nf-ml.html with data from nf-ml.txt"""
    html_file = Path("nf-ml.html")
...
<head>
  ...
  <title>NF-ML | Energy Data Summary</title>
...
<body>
  <div class="container">
    <header class="header">
      <h1>NF-ML: Naive Forecast (Persistence Model) vs Actual</h1>
      <nav class="header-nav">
        ...
        <a href="lr-ml.html" class="header-nav-link">LR-ML</a>
        <a href="rf-ml.html" class="header-nav-link">RF-ML</a>
        <a href="svr-ml.html" class="header-nav-link">SVR-ML</a>
        <a href="knn-ml.html" class="header-nav-link">KNN-ML</a>
      </nav>
    </header>
    ...
    <div class="main-summary">
      <div class="summary-cards">
        <div class="summary-card" style="grid-column: 1 / -1;">
          <div class="summary-label">NF Performance & Config</div><table style="width: 100%; border-collapse: collapse; font-size: 0.88em;">
            <thead>
...
<tr style="border-top: 2px solid rgba(255,255,255,0.1); padding: 8px 0;">
  ...
  <td style="text-align: left; font-weight: 600; color: #06b6d4; padding-left: 10px;" colspan="4">Naive Forecast (Persistence Model)</td>
 </tr>
 ...
 <tr style="padding: 4px 0 8px 0;">
  ...
  <td style="text-align: left; color: #06b6d4; font-weight: 600; padding-left: 10px;" colspan="4">Feb 21–28 2026 (8 days - repeats last observed pattern)</td>
 </tr>
...
def main():
    ...
    print(f"\nGenerating Naive Forecast predictions using {len(all_data)} total rows...")
    ml_daily = predict_naive_daily_avgs(all_data)
    ...
    with open(ml_output_file, "w") as f:
        ...
        f.write(f"# Summary: Training data = {total_api_rows} | full-api actual = {len(nasa_daily)} | NF predict = {len(ml_daily)}\n")
        f.write("[full-api]\n\n")
        
        # Data-A Header
        f.write("# Data-A: NF-ML Predictions (Feb 21–28 2026) - Persistence Model (repeats last observed)\n")
        ...
        if not common_dates:
            f.write("No overlapping dates between NF predictions and NASA data.\n")
        else:
            # Wind metrics
            ...
            corr_wind_raw = np.corrcoef(y_true_wind, y_pred_wind)[0,1]
            corr_wind = corr_wind_raw if not np.isnan(corr_wind_raw) else 0.0
            ...
            corr_wind_raw = np.corrcoef(y_true_wind, y_pred_wind)[0,1]
            corr_solar_raw = np.corrcoef(y_true_solar, y_pred_solar)[0,1]
            # Handle NaN correlation (occurs when all predictions are identical)
            corr_wind = corr_wind_raw if not np.isnan(corr_wind_raw) else 0.0
            corr_solar = corr_solar_raw if not np.isnan(corr_solar_raw) else 0.0
    ...
    print(f"✅ nf-ml.txt generated with Naive Forecast (Persistence Model) predictions")
    print(f"✅ nf-ml.html generated with hardcoded data from nf-ml.txt")
