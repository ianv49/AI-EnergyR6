...
ml_output_file = Path("data/knn-ml.txt")
...
def train_ml_model(hourly_data):
    ...
    # Train KNN models with k=5 neighbors
    # KNN parameters: n_neighbors=5 (number of neighbors to consider)
    wind_model = KNeighborsRegressor(n_neighbors=5, weights='distance')
    solar_model = KNeighborsRegressor(n_neighbors=5, weights='distance')
...
def generate_html(ml_daily, nasa_daily, metrics_text, total_api_rows):
    """Generate hardcoded knn-ml.html with data from knn-ml.txt"""
    html_file = Path("knn-ml.html")
...
<head>
  ...
  <title>KNN-ML | Energy Data Summary</title>
...
<body>
  <div class="container">
    <header class="header">
      <h1>KNN-ML: Predict vs Actual</h1>
    ...
    <div class="main-summary">
      <div class="summary-cards">
        <div class="summary-card" style="grid-column: 1 / -1;">
          <div class="summary-label">KNN Performance & Config</div><table style="width: 100%; border-collapse: collapse; font-size: 0.88em;">
            <thead>
...
<tr style="border-top: 2px solid rgba(255,255,255,0.1); padding: 8px 0;">
  ...
  <td style="text-align: left; font-weight: 600; color: #06b6d4; padding-left: 10px;" colspan="4">K-Nearest Neighbors (KNN) Regressor</td>
 </tr>
...
def main():
    ...
    print(f"\nTraining KNN model using {len(all_data)} total rows from all *api.txt files...")
    ...
    # Write to knn-ml.txt - FIXED SPACING
    total_api_rows = len(all_data)
    with open(ml_output_file, "w") as f:
        now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        f.write(f"# ML page output last updated: {now_str}\n")
        f.write(f"# Summary: Training data = {total_api_rows} | full-api actual = {len(nasa_daily)} | KNN predict = {len(ml_daily)}\n")
        f.write("[full-api]\n\n")
        
        # Data-A Header
        f.write("# Data-A: KNN-ML Predictions (Feb 21–28 2026)\n")
        ...
        if not common_dates:
            f.write("No overlapping dates between KNN predictions and NASA data.\n")
        else:
            # Wind metrics
    ...
    print(f"✅ knn-ml.txt generated with KNN ML (Data-A/B + metrics)")
    print(f"✅ knn-ml.html generated with hardcoded data from knn-ml.txt")
