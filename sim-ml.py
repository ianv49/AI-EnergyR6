...
sim_file      = Path("data/sim-api.txt")
ml_output_file = Path("data/sim-ml.txt")
...
def main():
    sim_data, _ = load_csv_skip_comments(sim_file)
    
    print("Training ML model using sim-api.txt data...")
    wind_model, solar_model, scaler = train_ml_model(sim_data)
    
    ml_daily = predict_daily_avgs(wind_model, solar_model, scaler)
    sim_daily = aggregate_daily_actual(sim_data)
    
    # Write to sim-ml.txt - FIXED SPACING
    with open(ml_output_file, "w") as f:
        now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        f.write(f"# ML page output last updated: {now_str}\n")
        f.write(f"# Summary: sim actual = {len(sim_daily)} | sim predict = {len(ml_daily)}\n")
        f.write("[sim]\n\n")
        ...
        # Data-B Header
        f.write("# Data-B: sim Power Daily Averages (Feb 21–28 2026)\n")
        f.write("id,timestamp,wind-min,wind-avg,wind-max,solar-min,solar-avg,solar-max,source\n")
        for row in sim_daily:
            f.write(f"{row['id']},{row['timestamp']},{row['wind-min']},{row['wind-avg']},{row['wind-max']},{row['solar-min']},{row['solar-avg']},{row['solar-max']},{row['source']}\n")
        f.write("\n")
        
        # Metrics
        f.write("# Metrics: Prediction vs sim Actual (Feb 21–28 2026)\n")
        pred_map = {r["timestamp"]: r for r in ml_daily}
        actual_map = {r["timestamp"]: r for r in sim_daily}
        common_dates = sorted(set(pred_map.keys()) & set(actual_map.keys()))

        if not common_dates:
            f.write("No overlapping dates between ML predictions and sim data.\n")
        else:
    ...
    print(f"✅ sim-ml.txt generated with sim ML (Data-A/B + metrics)")
