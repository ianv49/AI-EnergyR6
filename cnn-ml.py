...
ml_output_file = Path("data/cnn-ml.txt")
...
def create_sequences(X, y, seq_length=7):
    """Create sequences for CNN: (num_samples, seq_length, num_features)"""
    X_seq = []
    y_seq = []
...
def train_cnn_model(hourly_data, seq_length=24):
    ...
    # Create sequences
    X_seq, y_wind_seq = create_sequences(X_scaled, y_wind, seq_length)
    _, y_solar_seq = create_sequences(X_scaled, y_solar, seq_length)
    
    if len(X_seq) < 10:
        print("⚠️ Insufficient data for CNN training")
        return None, None, None
    
    # Train CNN model for wind
    wind_model = Sequential([
        Conv1D(filters=32, kernel_size=3, activation='relu', input_shape=(seq_length, X_scaled.shape[1])),
    ...
    wind_model.compile(optimizer=Adam(learning_rate=0.001), loss='mse', metrics=['mae'])
    wind_model.fit(X_seq, y_wind_seq, epochs=30, batch_size=16, verbose=0)
    
    # Train CNN model for solar
    ...
    solar_model.compile(optimizer=Adam(learning_rate=0.001), loss='mse', metrics=['mae'])
    solar_model.fit(X_seq, y_solar_seq, epochs=30, batch_size=16, verbose=0)
    
    return wind_model, solar_model, scaler

def predict_daily_avgs(model_wind, model_solar, scaler, seq_length=24):
    ...
    for i, date in enumerate(dates, 1):
        ...
        daily_hourly = np.array(daily_hourly)
        X_daily = scaler.transform(daily_hourly)
        
        # Use the first 24 hours to create a sequence for prediction
        if len(X_daily) >= seq_length:
            X_seq = np.expand_dims(X_daily[:seq_length], axis=0)
        else:
            # Pad if needed
            X_seq = np.pad(X_daily, ((0, seq_length - len(X_daily)), (0, 0)), mode='edge')
            X_seq = np.expand_dims(X_seq, axis=0)
        
        wind_preds = model_wind.predict(X_seq, verbose=0).flatten()
        solar_preds = model_solar.predict(X_seq, verbose=0).flatten()
        
        # Extend predictions for the full day
        wind_preds_full = np.tile(wind_preds, (max(1, 24 // len(wind_preds) + 1)))[:24]
        solar_preds_full = np.tile(solar_preds, (max(1, 24 // len(solar_preds) + 1)))[:24]
        
        wind_min, wind_avg, wind_max = min(wind_preds_full), np.mean(wind_preds_full), max(wind_preds_full)
        solar_min, solar_avg, solar_max = min(solar_preds_full), np.mean(solar_preds_full), max(solar_preds_full)
...
def generate_html(ml_daily, nasa_daily, metrics_text, total_api_rows):
    """Generate hardcoded cnn-ml.html with data from cnn-ml.txt"""
    html_file = Path("cnn-ml.html")
...
<head>
  ...
  <title>CNN-ML | Energy Data Summary</title>
...
<body>
  <div class="container">
    <header class="header">
      <h1>CNN-ML: Predict vs Actual</h1>
    ...
    <div class="main-summary">
      <div class="summary-cards">
        <div class="summary-card" style="grid-column: 1 / -1;">
          <div class="summary-label">CNN (1D Convolutional) Performance & Config</div><table style="width: 100%; border-collapse: collapse; font-size: 0.88em;">
            <thead>
...
<tr style="border-top: 2px solid rgba(255,255,255,0.1); padding: 8px 0;">
  ...
  <td style="text-align: left; font-weight: 600; color: #06b6d4; padding-left: 10px;" colspan="4">CNN (Conv1D: 32→64 filters, seq_len=7, epochs=50)</td>
 </tr>
...
def main():
    ...
    print(f"\nTraining CNN model using {len(all_data)} total rows from all *api.txt files...")
    wind_model, solar_model, scaler = train_cnn_model(all_data)
    
    if wind_model is None or solar_model is None:
        print("⚠️ CNN model training failed")
        return
    ...
    with open(ml_output_file, "w") as f:
        ...
        f.write(f"# Summary: Training data = {total_api_rows} | full-api actual = {len(nasa_daily)} | CNN predict = {len(ml_daily)}\n")
        f.write("[full-api]\n\n")
        
        # Data-A Header
        f.write("# Data-A: CNN Predictions (Feb 21–28 2026)\n")
        ...
        if not common_dates:
            f.write("No overlapping dates between CNN predictions and API data.\n")
        else:
            # Wind metrics
    ...
    print(f"✅ cnn-ml.txt generated with CNN ML (Data-A/B + metrics)")
    print(f"✅ cnn-ml.html generated with hardcoded data from cnn-ml.txt")
