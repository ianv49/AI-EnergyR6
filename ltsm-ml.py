...
ml_output_file = Path("data/ltsm-ml.txt")
...
def train_lstm_model(hourly_data):
    ...
    # Build LSTM model for wind
    wind_model = Sequential([
        LSTM(64, activation='relu', input_shape=(seq_length, 6), return_sequences=True),
        Dropout(0.2),
...
def generate_html(ml_daily, nasa_daily, metrics_text, total_api_rows):
    """Generate hardcoded ltsm-ml.html with data from ltsm-ml.txt"""
    html_file = Path("ltsm-ml.html")
...
<head>
  ...
  <title>LSTM-ML | Energy Data Summary</title>
...
<body>
  <div class="container">
    <header class="header">
      <h1>LSTM-ML: Predict vs Actual</h1>
    ...
    <div class="main-summary">
      <div class="summary-cards">
        <div class="summary-card" style="grid-column: 1 / -1;">
          <div class="summary-label">LSTM Performance & Config</div><table style="width: 100%; border-collapse: collapse; font-size: 0.88em;">
            <thead>
...
<tr style="border-top: 2px solid rgba(255,255,255,0.1); padding: 8px 0;">
  ...
  <td style="text-align: left; font-weight: 600; color: #06b6d4; padding-left: 10px;" colspan="4">LSTM (Long Short-Term Memory)</td>
 </tr>
...
def main():
    ...
    print(f"\nTraining LSTM model using {len(all_data)} total rows from all *api.txt files...")
    wind_model, solar_model, scaler, wind_scaler, solar_scaler = train_lstm_model(all_data)
    ...
    # Write to ltsm-ml.txt
    total_api_rows = len(all_data)
    with open(ml_output_file, "w") as f:
        now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        f.write(f"# ML page output last updated: {now_str}\n")
        f.write(f"# Summary: Training data = {total_api_rows} | full-api actual = {len(nasa_daily)} | LSTM predict = {len(ml_daily)}\n")
        f.write("[full-api]\n\n")
        
        # Data-A Header
        f.write("# Data-A: LSTM-ML Predictions (Feb 21–28 2026)\n")
    ...
    print(f"✅ ltsm-ml.txt generated with LSTM ML (Data-A/B + metrics)")
    print(f"✅ ltsm-ml.html generated with hardcoded data from ltsm-ml.txt")
