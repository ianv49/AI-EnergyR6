...
ml_output_file = Path("data/svr-ml.txt")
...
def train_ml_model(hourly_data):
    ...
    # Train SVR models with RBF kernel
    # SVR parameters: C=100 (regularization), gamma='scale' (kernel coefficient)
    wind_model = SVR(kernel='rbf', C=100, gamma='scale', epsilon=0.1)
    solar_model = SVR(kernel='rbf', C=100, gamma='scale', epsilon=0.1)
...
def generate_html(ml_daily, nasa_daily, metrics_text, total_api_rows):
    """Generate hardcoded svr-ml.html with data from svr-ml.txt"""
    html_file = Path("svr-ml.html")
...
<head>
  ...
  <title>SVR-ML | Energy Data Summary</title>
...
<body>
  <div class="container">
    <header class="header">
      <h1>SVR-ML: Predict vs Actual</h1>
    ...
    <div class="main-summary">
      <div class="summary-cards">
        <div class="summary-card" style="grid-column: 1 / -1;">
          <div class="summary-label">SVR Performance & Config</div><table style="width: 100%; border-collapse: collapse; font-size: 0.88em;">
            <thead>
...
<tr style="border-top: 2px solid rgba(255,255,255,0.1); padding: 8px 0;">
  ...
  <td style="text-align: left; font-weight: 600; color: #06b6d4; padding-left: 10px;" colspan="4">Support Vector Regression (SVR)</td>
 </tr>
...
def main():
    ...
    print(f"\nTraining SVR model using {len(all_data)} total rows from all *api.txt files...")
    ...
    # Write to svr-ml.txt - FIXED SPACING
    total_api_rows = len(all_data)
    with open(ml_output_file, "w") as f:
        now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        f.write(f"# ML page output last updated: {now_str}\n")
        f.write(f"# Summary: Training data = {total_api_rows} | full-api actual = {len(nasa_daily)} | SVR predict = {len(ml_daily)}\n")
        f.write("[full-api]\n\n")
        
        # Data-A Header
        f.write("# Data-A: SVR-ML Predictions (Feb 21–28 2026)\n")
        ...
        if not common_dates:
            f.write("No overlapping dates between SVR predictions and NASA data.\n")
        else:
            # Wind metrics
    ...
    print(f"✅ svr-ml.txt generated with SVR ML (Data-A/B + metrics)")
    print(f"✅ svr-ml.html generated with hardcoded data from svr-ml.txt")
