...
def prepare_training_set():
    ...
    files = [
        'data/collect1.txt', 'data/collect2.txt', 'data/collect3.txt', 'data/collect4.txt',
...
def load_historical_rf_data(file='data/rf-api.txt'):\n    \"\"\"Load historical rf data for training (from rf-ml-forecast.py)\"\"\"
    try:
        df = pd.read_csv(file, skiprows=3)
        df['timestamp'] = pd.to_datetime(df['timestamp'])
        df = df[(df['timestamp'] >= '2025-01-01') & (df['timestamp'] < '2026-02-21')]
        df = df.dropna()
        logger.info(f"Loaded {len(df)} historical rf records")
        return df
    except Exception as e:
        logger.error(f"Error loading rf data: {e}")
        return pd.DataFrame()
...
def build_validation_charts(df_ml, df_api):
    ...
    df_api_daily['type'] = 'RF API Actual'
...
def ml_forecast_pipeline():
    """Full ML forecast"""
    df_hist = load_historical_rf_data()
    if df_hist.empty:
        return
